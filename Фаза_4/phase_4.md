...
**Идея модели.**  
История по фактически выделяемому времени на предмет (человеко-часы в неделю) рассматривается как **дискретное распределение** случайной величины `hours_per_week`.  
При этом для каждой задачи Фазы 5 задаётся трёхточечная оценка длительности `a/m/b` (optimistic/most likely/pessimistic), а длительность задачи в каждом прогоне семплируется из **треугольного распределения** `triangular(a, m, b)`.

В каждом прогоне:
1) семплируются длительности всех задач из `triangular(a,m,b)`;
2) по неделям семплируется доступная мощность `hours_per_week`;
3) задачи «выполняются» последовательно, расходуя недельные часы, пока все задачи не будут завершены.

**Что получаем на выходе:**
- распределение сроков завершения Фазы 5 (в неделях);
- оценку срока с **80% достоверностью (P80)**.
